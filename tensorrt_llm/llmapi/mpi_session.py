...
from .._cache import (_DERIVED_CACHE_ENV_VARS, UNIFIED_CACHE_ROOT_ENV,
                      cache_bootstrap_path, derived_cache_env_vars)
    ...
    def _start_mpi_pool(self):
        ...
        env = {
            ...
            if key.startswith("TRTLLM") or key.startswith("TLLM") or key in (
                ...
                "FLASHINFER_CUBIN_DIR", "CUTE_DSL_CACHE_DIR",
                "DG_JIT_CACHE_DIR", "CUDA_CACHE_PATH")
        }
        env.update(self._env_overrides)
        unified_cache = env.get(UNIFIED_CACHE_ROOT_ENV)
        if unified_cache:
            # The parent was configured as rank 0. Do not apply those derived
            # values after the worker bootstrap reranks them.
            derived = derived_cache_env_vars() - self._env_overrides.keys()
            env[_DERIVED_CACHE_ENV_VARS] = ",".join(sorted(derived))
            for name in derived:
                env.pop(name, None)
        ...
        if unified_cache:
            python_args = [cache_bootstrap_path(), "unified"]
        elif isolate_workspace:
            python_args = [
                cache_bootstrap_path(), "flashinfer-isolate",
